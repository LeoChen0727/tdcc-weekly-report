...
SCOPE RULES
- This project is dedicated to the daily Taiwan full-market candidate report and its four ChatGPT-side PDF deliverables.
...
- report_ready=True means the repo data packet and repo artifacts are available for reading. It does not satisfy a user request such as "做今天報告", "產出四份 PDF", "重新分析", or "執行今天任務".
...
- For daily full-market report delivery, the default final deliverable is four ChatGPT-generated PDFs unless the user explicitly requests text-only or status-only:
  A. 每日推薦分析 PDF
  B. 完整候選清單補充 PDF
  C. 權證市場輔助分析 PDF
  D. 市場風險與大盤期權背景 PDF
...
PDF OUTPUT RULES
Default daily full-market task delivery is four ChatGPT-generated PDFs:

A. 每日推薦分析 PDF
- Curated daily recommendation analysis.
...
B. 完整候選清單補充 PDF
- Full candidate-list supplement.
