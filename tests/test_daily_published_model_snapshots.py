...
def test_pr_immutable_history_audit_is_independent_of_current_publish_readiness(
    tmp_path: Path,
) -> None:
    latest_dir = tmp_path / "output" / "latest"
    snapshot_dir = tmp_path / "output" / "history" / "daily_model_snapshots"
    ...
    freshness.loc[0, "report_ready"] = "False"
    freshness.loc[0, "daily_pdf_ready"] = "False"
    freshness.to_csv(
        freshness_path,
        index=False,
        encoding="utf-8",
    ...
    current_errors = validate_snapshots.validate_current_report_snapshots(
        latest_dir=latest_dir,
        snapshot_dir=snapshot_dir,
        manifest_path=manifest_path,
    )
    assert any(
        "report_ready must be True before publishing model snapshots" in error
        for error in current_errors
    )

    (latest_dir / "daily_candidate_model_signals_for_report_latest.csv").unlink()
    assert validate_snapshots.validate_pr_immutable_snapshot_history(
        snapshot_dir=snapshot_dir,
        manifest_path=manifest_path,
    ) == []


def test_pr_immutable_history_audit_retains_snapshot_and_publish_flag_guards(
    tmp_path: Path,
) -> None:
    latest_dir = tmp_path / "output" / "latest"
    snapshot_dir = tmp_path / "output" / "history" / "daily_model_snapshots"
    ...
    manifest = pd.read_csv(manifest_path, dtype=str).fillna("")
    snapshot_path = repository_file(tmp_path, manifest.iloc[0]["snapshot_path"])
    snapshot_payload = snapshot_path.read_bytes()

    snapshot_path.write_bytes(snapshot_payload + b"tampered\n")
    tamper_errors = validate_snapshots.validate_pr_immutable_snapshot_history(
        snapshot_dir=snapshot_dir,
        manifest_path=manifest_path,
    )
    assert any("snapshot_sha256 does not match snapshot file" in error for error in tamper_errors)
    snapshot_path.write_bytes(snapshot_payload)

    manifest_payload = manifest_path.read_bytes()
    for column in ("report_ready", "daily_pdf_ready"):
        modified_manifest = manifest.copy()
        modified_manifest.loc[0, column] = "False"
        modified_manifest.to_csv(
            manifest_path,
            index=False,
            encoding="utf-8",
            lineterminator="\n",
        )
        flag_errors = validate_snapshots.validate_pr_immutable_snapshot_history(
            snapshot_dir=snapshot_dir,
            manifest_path=manifest_path,
        )
        assert any(f"{column} must be True" in error for error in flag_errors)
        manifest_path.write_bytes(manifest_payload)


def test_snapshot_validator_cli_separates_pr_history_mode_from_scoped_current_mode() -> None:
    default_args = validate_snapshots.parse_args([])
    assert default_args.pr_immutable_history_only is False
    assert default_args.artifact_id is None

    history_args = validate_snapshots.parse_args(["--pr-immutable-history-only"])
    assert history_args.pr_immutable_history_only is True
    assert history_args.artifact_id is None

    with pytest.raises(SystemExit):
        validate_snapshots.parse_args(
            ["--pr-immutable-history-only", "--artifact-id", "data_freshness"]
        )
