...
def test_tdcc_stealth_accumulation_outcome_owner_is_exact_and_local(mutation):
    rows, owned = validator.load_registry(), validator.load_model_owned_producers()
    owner = validator.TDCC_OUTCOME_UNIT_LOCAL_OWNER
    producer = validator.TDCC_OUTCOME_UNIT_LOCAL_PRODUCER
...
def test_tdcc_stealth_accumulation_outcome_producer_remote_blocked_validator_allowed(path):
    assert validator.validate_annual_local_workflow_exclusion({
        path: "run: python " + validator.TDCC_OUTCOME_UNIT_LOCAL_PRODUCER
    })
    assert validator.validate_annual_local_workflow_exclusion({
