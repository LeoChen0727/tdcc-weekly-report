...
def _price_frame(
    *,
    ...
    for trigger_date in trigger_dates:
        hits = frame.index[frame["date"].eq(trigger_date)]
        if not len(hits):
            continue
        trigger_index = int(hits[0])
        frame.at[trigger_index, "cross_breakout_prev20"] = True
        frame.at[trigger_index, "analysis_close"] = 101.0
        frame.at[trigger_index, "close"] = 101.0
        if trigger_index + 1 < len(frame):
            frame.at[trigger_index + 1, "analysis_close"] = 103.0
            frame.at[trigger_index + 1, "close"] = 103.0
...
def test_pr462_projection_is_exactly_pinned_and_drift_fails_closed() -> None:
    ...
    for column, corrupt_value in (
        ...
        with pytest.raises(RuntimeError, match="PR462 projected episode"):
            build_forward_holdout(
                source,
                daily,
...
def test_writer_keeps_byte_equal_mirrors_inside_research_only_scope(
    ...
    daily["1111"].at[last, "analysis_close"] += 0.01
    daily["1111"].at[last, "close"] += 0.01
    ...
    write_forward_holdout(*next_frames, output_root=tmp_path)
...
def test_late_history_rewrite_failure_leaves_every_prior_surface_unchanged(
    ...
    source, daily, source_manifest = holdout_inputs()
    last = daily["1111"].index[-1]
    daily["1111"].at[last, "analysis_close"] += 0.02
    daily["1111"].at[last, "close"] += 0.02
...
def test_injected_publish_io_failure_rolls_back_all_15_paths_byte_exact(
    ...
    source, daily, source_manifest = holdout_inputs()
    last = daily["1111"].index[-1]
    daily["1111"].at[last, "analysis_close"] += 0.03
    daily["1111"].at[last, "close"] += 0.03
    next_frames = build_forward_holdout(
        source,
        daily,
        source_manifest=source_manifest,
...
def test_post_publish_replay_failure_rolls_back_all_15_paths_byte_exact(
    ...
    source, daily, source_manifest = holdout_inputs()
    last = daily["1111"].index[-1]
    daily["1111"].at[last, "analysis_close"] += 0.04
    daily["1111"].at[last, "close"] += 0.04
    next_frames = build_forward_holdout(
        source,
        daily,
        source_manifest=source_manifest,
