...
New formal daily models must use a model-owned production module or producer by default. The existing `scripts/build_daily_candidate_model_layer.py` monolith is a contained legacy surface and must not gain a new model without explicit user approval. Every active model must register exact runtime entries, reachable function/global AST ownership, and a canonical semantic SHA in `config/daily_model_semantic_ownership.csv`. Every semantic item consumed by multiple models must be listed with exact consumers in `config/daily_model_shared_semantic_registry.csv`. A new consumer or semantic change requires an append-only user-approved migration in `config/daily_model_semantic_migrations.csv`; updating a baseline hash alone is forbidden.

Every new database, feature panel, snapshot family, and model research artifact must be registered before a producer or model may consume it. `config/daily_model_data_sharing_registry.csv` must identify one writer, the exact write scope, point-in-time status, approved read consumers, forbidden uses, retention policy, and a user-approved sharing decision. Model-owned outputs remain model-owned even when their raw inputs are shared objective data. Adding a writer, consumer, or cross-model use without a new approval reference and append-only `config/daily_model_data_sharing_migrations.csv` row must fail validation.

Independent promotion evidence validators must not import the production business functions they claim to verify. A validator that imports model conditions, scores, ranking, or model-owned feature interpretation is an implementation-consistency audit only and cannot be cited as independent backtest or promotion evidence. This boundary is registered in `config/daily_model_validator_independence.csv` and enforced by `scripts/validate_model_data_independence.py`.
