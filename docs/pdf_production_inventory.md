...
| TDCC weekly candidate reports | `scripts/run_tdcc_weekly_report_entrypoint.py` -> `scripts/build_tdcc_weekly_candidate_reports.py` | `scripts/validate_tdcc_weekly_candidate_reports.py`, `scripts/validate_tdcc_weekly_pdf_font_contract.py`, `scripts/validate_pdf_facing_display_text.py` | `output/latest`, `output/history`, and `docs/latest` through `tdcc_weekly.yml`; Codex automation should use the entrypoint so local generated artifacts do not block clean-source builds. |
