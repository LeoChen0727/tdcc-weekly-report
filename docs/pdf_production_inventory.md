...
- GitHub Windows DFKai replay runs only for true renderer/font-contract pull requests through `daily_pdf_replay_pr_validation.yml` or an explicitly authorized renderer/font investigation. Formal six-PDF delivery remains owned by Daily PDF Production V1 through the official local entrypoint after Daily Full publishes ready authority surfaces. Model-produced operation artifacts, general model research, financial-statement source-audit changes, and routine Daily Full runs stay off the Windows replay path.
