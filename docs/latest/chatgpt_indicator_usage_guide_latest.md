...
- generated_at: `2026-06-18 05:50:59 UTC`
...
| Warrant flow | output/latest/warrant_flow_by_stock_latest.csv | warrant_flow_signal, warrant_flow_score, warrant_flow_warning | 無明確權證訊號=309; 認購流入=73; 認購/認售結構偏多=31; 認購強流入=30; 認售流入=6; 多空混合=2 | Auxiliary only. Do not make warrant-only conclusions. |
...
| Daily theme status history | output/history/daily_signals/daily_theme_status_history.csv | signal_date, stock_id, theme_final_status, theme_status_group, theme_volume_attack_status, candidate_source_type | mainstream_overheated=207; mainstream_supported=150; non_mainstream=4 / rows=361 | Use for no-lookahead mainstream/non-mainstream backtests; do not use today's theme label for older signal dates. |
