...
- generated_at: `2026-07-04 14:40:41 Asia/Taipei`
...
| operation_candidate_v1_pending_promotion_pr | 1 |
...
| blocked_promotion_pr_and_daily_operation_adapter_required | 1 |
...
| price_pullback_23ema | production_proxy | operation_candidate_v1_pending_promotion_pr | blocked_promotion_pr_and_daily_operation_adapter_required | False | pending_promotion_pr_and_daily_adapter | price_pullback_23ema_prev20_breakout_stop_v1 | price_pullback_23ema_operation_candidate_v1_20260630 | False | no_operation_directive | blocked_promotion_pr_and_daily_operation_adapter_required | blocked_promotion_pr_and_daily_operation_adapter_required | daily row parity audit has no failing rows, but an explicit promotion/sync PR and formal daily operation-row adapter/PDF section contract are still required before production operation guidance | price_pullback_23ema 已選出 operation candidate v1：先有 production proxy 訊號，且同日符合大戶高門檻增加與 20 日漲幅 0% 到 25%；買點為次日開盤，勝利為 D+20 前盤中突破訊號日前 20 日高點，失敗為連續 4 日收盤低於 MA20/EMA23 較低者 4%。daily row parity 已通過；仍需要獨立 promotion/sync PR 與正式 daily operation-row adapter/PDF section contract 才能進 production。PDF renderer 不得自行推論 23EMA 操作列。daily row parity audit 已通過 13 個 published snapshots。所以不得產生 production 買進、賣出、停損或排名操作建議。 |
