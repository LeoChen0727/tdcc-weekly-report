...
Formal operation-adapter status fields are protected contracts, not free-form descriptions. Every mature model must register canonical lifecycle states and allowed values in `config/daily_operation_adapter_protected_field_contract.csv`. A producer must write `quality_status_zh` or `operation_quality`, `operation_status`, `operation_status_zh`, `row_action_status`, `buy_rank_eligible`, and `adapter_note_zh` exactly once per row-construction function. Model-specific explanation belongs in `adapter_note_zh` and must not overwrite canonical quality or lifecycle fields. Duplicate protected dict keys, repeated protected-field writes, and repeated protected module constants must fail AST validation. Every supported lifecycle state requires synthetic non-empty regression coverage, and actual builder rows must pass both the model validator and `scripts/validate_daily_operation_adapter_protected_fields.py`. An empty-state workflow run cannot prove non-empty lifecycle coverage.

Numerical magnitude is an investigation trigger, not an anomaly disposition. An absolute-return cutoff, percentile, IQR, z-score, trimmed mean, winsorized result, or contribution to an average may create only an `anomaly_candidate`; it cannot by itself prove an extreme value, data error, non-comparable event, exclusion, or corrected performance. Final disposition requires bottom-level evidence covering identity/non-overlap, formal operation replay, point-in-time and trading-calendar continuity, raw-source lineage and immutable hashes, units/formulas/adjustment basis, authoritative corporate-action or business-event history, independent-source corroboration, and a reproducible evidence reference. Missing evidence means `unresolved_anomaly_candidate`: the row remains in primary metrics and the affected model promotion conclusion is blocked. A `verified_real_extreme` remains in primary metrics; a `verified_data_error` must be repaired at source and rerun; a `verified_non_comparable` may be excluded only with an approved reason and rerun. Threshold-excluded metrics are sensitivity analysis only and cannot replace primary performance. The machine contract is `config/daily_model_numerical_anomaly_disposition_contract.csv` and is enforced by `scripts/validate_repo_code_isolation_policy.py`.

Legacy research fields or artifacts labeled `anomaly`, `outlier`, or `extreme` from only a threshold or quantile are retroactively `anomaly_candidate` regardless of their old names. They are forbidden as final anomaly evidence or pinned formal promotion evidence and must be republished under the root-cause disposition contract before the model is reopened or promoted.
