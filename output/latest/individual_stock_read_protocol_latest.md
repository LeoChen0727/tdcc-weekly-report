...
2. Read the individual ChatGPT packet first. It embeds a compact 180-day OHLCV window, recent TDCC rows, candidate context, repeat appearance, and warrant context.
3. If packet raw/pages returns Cache miss, Internal Error, stale content, or `Total lines: 1`, read the GitHub API packet URL and base64-decode `content`.
4. Use the packet for ordinary single-stock analysis. Use full price / TDCC raw CSV only when a deeper backtest or extra columns are needed.
5. If raw price history returns Cache miss, Internal Error, stale content, or `Total lines: 1` while the file is expected to be multi-line, read the GitHub API price URL and base64-decode `content`.
6. If price rows >= 60, the stock can be analyzed as `standard_rawdata_report` even when the individual Markdown/PDF report does not exist.
7. Read TDCC history from the packet first, then TDCC raw/API fallback if needed.
8. If TDCC rows < 8 weekly rows, mark `insufficient_tdcc_history`; do not make 8-12 week TDCC backtest conclusions.
9. Read individual report Markdown only as an optional prepared report. If it is missing, continue with packet/raw data.
10. External websites may supplement news, announcements, broker targets, or events, but must not replace repo price history or repo TDCC history as primary data.
