# TDCC Phase Distribution

- generated_at: 2026-07-11 18:24:32 Asia/Taipei
- latest_signal_count: 1118
- phase_mature_d5_count: 307
- phase_mature_d10_count: 265
- phase_mature_d20_count: 160

## Phase 分布

| tdcc_price_phase | sample_count | pct_of_total |
| --- | --- | --- |
| insufficient_price_context | 798.0 | 71.38 |
| tdcc_leading_price | 116.0 | 10.38 |
| tdcc_price_divergence | 116.0 | 10.38 |
| price_leading_tdcc | 45.0 | 4.03 |
| overheated_after_tdcc | 33.0 | 2.95 |
| tdcc_price_confirmed | 8.0 | 0.72 |
| failed_after_tdcc | 2.0 | 0.18 |

## 連續週數 x Phase

| tdcc_consecutive_up_weeks | tdcc_price_phase | signal_count |
| --- | --- | --- |
| 1 | insufficient_price_context | 418.0 |
| 1 | overheated_after_tdcc | 12.0 |
| 1 | price_leading_tdcc | 14.0 |
| 2 | failed_after_tdcc | 2.0 |
| 2 | insufficient_price_context | 156.0 |
| 2 | overheated_after_tdcc | 7.0 |
| 2 | price_leading_tdcc | 9.0 |
| 2 | tdcc_leading_price | 35.0 |
| 2 | tdcc_price_confirmed | 4.0 |
| 2 | tdcc_price_divergence | 31.0 |
| 3 | insufficient_price_context | 96.0 |
| 3 | overheated_after_tdcc | 9.0 |
| 3 | price_leading_tdcc | 13.0 |
| 3 | tdcc_leading_price | 22.0 |
| 3 | tdcc_price_confirmed | 1.0 |
| 3 | tdcc_price_divergence | 20.0 |
| 4 | insufficient_price_context | 43.0 |
| 4 | overheated_after_tdcc | 1.0 |
| 4 | price_leading_tdcc | 2.0 |
| 4 | tdcc_leading_price | 12.0 |
| 4 | tdcc_price_confirmed | 1.0 |
| 4 | tdcc_price_divergence | 19.0 |
| 5 | insufficient_price_context | 26.0 |
| 5 | price_leading_tdcc | 3.0 |
| 5 | tdcc_leading_price | 11.0 |
| 5 | tdcc_price_confirmed | 1.0 |
| 5 | tdcc_price_divergence | 17.0 |
| 6 | insufficient_price_context | 14.0 |
| 6 | overheated_after_tdcc | 1.0 |
| 6 | price_leading_tdcc | 1.0 |
| 6 | tdcc_leading_price | 5.0 |
| 6 | tdcc_price_divergence | 9.0 |
| 7 | insufficient_price_context | 45.0 |
| 7 | overheated_after_tdcc | 3.0 |
| 7 | price_leading_tdcc | 3.0 |
| 7 | tdcc_leading_price | 31.0 |
| 7 | tdcc_price_confirmed | 1.0 |
| 7 | tdcc_price_divergence | 20.0 |

## TDCC 條件 x Phase

| condition_name | tdcc_price_phase | signal_count |
| --- | --- | --- |
| all_thresholds_up | insufficient_price_context | 145.0 |
| all_thresholds_up | tdcc_leading_price | 102.0 |
| all_thresholds_up | tdcc_price_divergence | 99.0 |
| all_thresholds_up | price_leading_tdcc | 39.0 |
| all_thresholds_up | overheated_after_tdcc | 31.0 |
| all_thresholds_up | tdcc_price_confirmed | 7.0 |
| all_thresholds_up | failed_after_tdcc | 2.0 |
| high_thresholds_up | insufficient_price_context | 238.0 |
| high_thresholds_up | tdcc_leading_price | 116.0 |
| high_thresholds_up | tdcc_price_divergence | 116.0 |
| high_thresholds_up | price_leading_tdcc | 45.0 |
| high_thresholds_up | overheated_after_tdcc | 33.0 |
| high_thresholds_up | tdcc_price_confirmed | 8.0 |
| high_thresholds_up | failed_after_tdcc | 2.0 |
| over_800_or_above | insufficient_price_context | 496.0 |
| over_800_or_above | tdcc_leading_price | 116.0 |
| over_800_or_above | tdcc_price_divergence | 116.0 |
| over_800_or_above | price_leading_tdcc | 45.0 |
| over_800_or_above | overheated_after_tdcc | 33.0 |
| over_800_or_above | tdcc_price_confirmed | 8.0 |
| over_800_or_above | failed_after_tdcc | 2.0 |
| over_1000_only | insufficient_price_context | 95.0 |
| consecutive_2w | insufficient_price_context | 380.0 |
| consecutive_2w | tdcc_leading_price | 116.0 |
| consecutive_2w | tdcc_price_divergence | 116.0 |
| consecutive_2w | price_leading_tdcc | 31.0 |
| consecutive_2w | overheated_after_tdcc | 21.0 |
| consecutive_2w | tdcc_price_confirmed | 8.0 |
| consecutive_2w | failed_after_tdcc | 2.0 |
| consecutive_3w | insufficient_price_context | 224.0 |
| consecutive_3w | tdcc_price_divergence | 85.0 |
| consecutive_3w | tdcc_leading_price | 81.0 |
| consecutive_3w | price_leading_tdcc | 22.0 |
| consecutive_3w | overheated_after_tdcc | 14.0 |
| consecutive_3w | tdcc_price_confirmed | 4.0 |
| quiet_accumulation | tdcc_price_divergence | 70.0 |
| quiet_accumulation | tdcc_leading_price | 69.0 |
| quiet_accumulation | insufficient_price_context | 58.0 |
| quiet_accumulation | tdcc_price_confirmed | 2.0 |
| quiet_accumulation | price_leading_tdcc | 2.0 |
| early_breakout | tdcc_leading_price | 1.0 |
| early_breakout | insufficient_price_context | 1.0 |
| early_breakout | tdcc_price_confirmed | 1.0 |
| strong_momentum | price_leading_tdcc | 9.0 |
| strong_momentum | insufficient_price_context | 8.0 |
| strong_momentum | tdcc_leading_price | 3.0 |
| strong_momentum | tdcc_price_divergence | 2.0 |
| strong_momentum | tdcc_price_confirmed | 2.0 |
| overheated | overheated_after_tdcc | 33.0 |
| overheated | price_leading_tdcc | 13.0 |
| overheated | insufficient_price_context | 3.0 |
| overheated | tdcc_price_divergence | 3.0 |
| overheated | tdcc_price_confirmed | 1.0 |

## Phase 後續成熟績效



| tdcc_price_phase | mature_sample_d5 | avg_ret_d5 | avg_relative_ret_d5 | mature_sample_d10 | avg_ret_d10 | avg_relative_ret_d10 | mature_sample_d20 | avg_ret_d20 | avg_relative_ret_d20 | avg_mfe_d10 | avg_mae_d10 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| failed_after_tdcc | 0.0 |  |  | 0.0 |  |  | 0.0 |  |  |  |  |
| insufficient_price_context | 93.0 | 2.52 | 0.24 | 82.0 | 3.00 | 1.00 | 40.0 | 3.00 | -0.77 | 15.99 | -8.76 |
| overheated_after_tdcc | 104.0 | 4.36 | 2.42 | 96.0 | 6.95 | 4.21 | 65.0 | 12.72 | 7.98 | 19.68 | -9.90 |
| price_leading_tdcc | 76.0 | 7.14 | 4.35 | 59.0 | 4.49 | 2.60 | 35.0 | 6.95 | 3.87 | 17.92 | -8.36 |
| tdcc_leading_price | 19.0 | -0.10 | 0.30 | 17.0 | 2.43 | -2.67 | 14.0 | 1.85 | -1.37 | 10.73 | -9.31 |
| tdcc_price_confirmed | 6.0 | -3.57 | -3.78 | 4.0 | -0.94 | -6.69 | 4.0 | -4.97 | -7.95 | 6.37 | -14.03 |
| tdcc_price_divergence | 9.0 | -3.54 | -4.26 | 7.0 | -1.49 | -4.62 | 2.0 | 2.49 | 0.44 | 10.05 | -12.95 |
