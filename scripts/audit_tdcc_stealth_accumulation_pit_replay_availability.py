...
OWNER_ID = "tdcc_stealth_accumulation_pit_replay_availability_audit"
PRODUCER = "scripts/audit_tdcc_stealth_accumulation_pit_replay_availability.py"
ARTIFACT_VERSION = "tdcc_stealth_accumulation_pit_replay_availability_audit_v1"
ARTIFACT_NAME = f"{ARTIFACT_VERSION}.csv"
DEFAULT_OUTPUT = ROOT / "output" / "research" / MODEL_ID / ARTIFACT_NAME
AVAILABILITY_STATE = (
...
class SourceAccess:
    """Read tracked sources from a full checkout or the selected Git tree.

    Sparse task worktrees intentionally omit protected data and output roots.  The
    Git fallback inventories those already-committed sources without materializing
    or rewriting them.
    """
...
def _hash_contract(
    access: SourceAccess, paths: list[str]
) -> tuple[str, str]:
    if not paths:
        return "", ""
    if len(paths) == 1:
        return "raw_file_sha256", _raw_sha256(access, paths[0])
    return "sorted_path_tab_raw_file_sha256_lf_aggregate_sha256", _family_sha256(
        access, paths
    )
...
def _published_snapshot_metrics(
    ...
    manifest_fields, manifest_rows = _read_csv(access, MANIFEST_PATH)
    missing = sorted(MANIFEST_REQUIRED_FIELDS - set(manifest_fields))
    if missing:
        raise RuntimeError(f"published snapshot manifest missing fields: {missing}")
    scoped_by_artifact = {
        artifact_id: [
            row for row in manifest_rows if row.get("artifact_id") == artifact_id
        ]
    ...
    def select(artifact_id: str) -> dict[str, Any]:
        scoped_rows = scoped_by_artifact[artifact_id]
        ...
        for report_date in sorted(grouped):
            ...
            revision_numbers = [
                _revision_number(row["snapshot_revision"]) for row in revisions
            ]
            ...
            snapshot_path = _normal_relative_path(selected["snapshot_path"])
            payload = access.read_bytes(snapshot_path)
            expected_sha = _text(selected["snapshot_sha256"]).lower()
            if re.fullmatch(r"[0-9a-f]{64}", expected_sha) is None:
                raise RuntimeError(f"invalid snapshot SHA-256: {snapshot_path}")
            if expected_sha not in _hash_candidates(payload):
                raise RuntimeError(f"snapshot SHA-256 mismatch: {snapshot_path}")
            if hashlib.sha256(payload).hexdigest() != expected_sha:
                transport_normalized_hash_count += 1
            fields, rows = _read_csv(access, snapshot_path)
            ...
            if missing_fields:
                raise RuntimeError(
                    ...
                    f"at {snapshot_path}: {missing_fields}"
                )
            for row_number, snapshot_row in enumerate(rows, start=2):
                ...
                if re.fullmatch(r"[0-9]{8}", signal_date) is None:
                    raise RuntimeError(
                        ...
                        f"at {snapshot_path}:{row_number}: {signal_date!r}"
                    )
                if signal_date != report_date:
                    raise RuntimeError(
                        f"{artifact_id} snapshot identity signal_date does not "
                        f"match manifest report date at {snapshot_path}:{row_number}"
                    )
                if not stock_id:
                    raise RuntimeError(
                        f"{artifact_id} snapshot identity stock_id is blank "
                        f"at {snapshot_path}:{row_number}"
                    )
                if (
            ...
            try:
                expected_rows = int(_text(selected["row_count"]))
                expected_columns = int(_text(selected["column_count"]))
            except ValueError as exc:
                raise RuntimeError(
                    f"invalid snapshot coverage count: {snapshot_path}"
                ) from exc
            if expected_rows != len(rows) or expected_columns != len(fields):
                raise RuntimeError(f"snapshot coverage count mismatch: {snapshot_path}")
            selected_paths.append(snapshot_path)
            selected_rows.extend(rows)
            selected_field_sets.append(set(fields))
    ...
    snapshot_metrics = select(MANIFEST_ARTIFACT_ID)
    all_candidates_metrics = select(ALL_CANDIDATES_ARTIFACT_ID)
    for artifact_id, metrics in (
        (ALL_CANDIDATES_ARTIFACT_ID, all_candidates_metrics),
        (MANIFEST_ARTIFACT_ID, snapshot_metrics),
    ):
        observed = (
...
def _csv_metrics(
    ...
    path: str,
    *,
    date_fields: tuple[str, ...] = ("signal_date", "date"),
    ...
    return {
        "paths": [path],
        "row_count": len(rows),
        "dates": _row_dates(scoped_rows, date_fields),
...
def _dated_family_metrics(
    ...
    *,
    ...
    paths = [
        path
        for path in access.list_paths(directory, pattern)
        if _date_from_filename(path, filename_prefix)
    ]
    dates = {_date_from_filename(path, filename_prefix) for path in paths}
    row_count = 0
    stocks: set[str] = set()
    ...
    if read_rows:
        for offset in range(0, len(paths), 64):
            ...
            for path in batch:
                fields, source_rows = _read_csv_payload(payloads[path], path)
                field_orders.add(tuple(fields))
                row_count += len(source_rows)
                stocks.update(_row_stocks(source_rows))
                identities.update(
...
def _per_stock_family_metrics(
    access: SourceAccess, directory: str, *, read_rows: bool = False
) -> dict[str, Any]:
    paths = [
        path
        for path in access.list_paths(directory, "*.csv")
...
def _tdcc_latest_dataset_manifest_metrics(access: SourceAccess) -> dict[str, Any]:
    ...
    def exact_dates(values: list[Any], field: str) -> set[str]:
        ...
        invalid = [value for value in normalized if re.fullmatch(r"[0-9]{8}", value) is None]
        if invalid:
            raise RuntimeError(
...
def _source_row(
    *,
    ...
    return {
        ...
        "source_row_count": (
            "" if metrics.get("row_count") is None else str(metrics["row_count"])
        ),
        ...
        "selector_missing_inputs": _text(metrics.get("selector_missing_inputs")),
        ...
        "coverage_status": coverage_status if available else "missing_source",
        "coverage_detail": _text(metrics.get("coverage_detail")),
        "pit_status": pit_status if available else "unavailable",
...
def build_audit_rows(
    repository_root: Path = ROOT, source_ref: str = "HEAD"
) -> list[dict[str, str]]:
    access = SourceAccess(repository_root, source_ref)
    selector_inputs = _selector_required_inputs(access)
    manifest_metrics, all_candidates_metrics, snapshot_metrics = (
        _published_snapshot_metrics(access, selector_inputs)
    )
    published_target_count = int(snapshot_metrics["target_count"])
    blockers = BASE_BLOCKERS + (
        (ZERO_SAMPLE_BLOCKER,) if published_target_count == 0 else ()
    )

    legacy_metrics = _csv_metrics(
        access, LEGACY_SIGNAL_LOG_PATH, target_model_rows=True
    )
    weekly_metrics = _dated_family_metrics(
        access,
        directory="output/history/tdcc",
        pattern="tdcc_holder_ratio_*.csv",
        filename_prefix="tdcc_holder_ratio_",
        read_rows=True,
    )
    raw_metrics = _dated_family_metrics(
        access,
        directory="output/history/tdcc",
        pattern="tdcc_latest_ratio_raw_*.csv",
        filename_prefix="tdcc_latest_ratio_raw_",
        read_rows=True,
    )
    latest_dataset_metrics = _tdcc_latest_dataset_manifest_metrics(access)
    dataset_manifest_metrics = _dated_family_metrics(
        access,
        directory="output/history/tdcc",
        pattern="tdcc_dataset_manifest_*.json",
        filename_prefix="tdcc_dataset_manifest_",
        read_rows=False,
    )
    tdcc_signal_metrics = _csv_metrics(access, TDCC_SIGNAL_SNAPSHOT_PATH)
    tdcc_per_stock_metrics = _per_stock_family_metrics(
        access, "data/tdcc_stock_history_raw", read_rows=True
    )
    tdcc_normalized_metrics = _per_stock_family_metrics(
        access, "data/tdcc_stock_history", read_rows=True
    )
    daily_price_metrics = _daily_price_metrics(access)
    stock_price_metrics = _per_stock_family_metrics(
        access, "data/stock_price_history", read_rows=True
    )
    contract_metrics = _stock_model_contract_metrics(access)
    semantic_metrics = _semantic_metrics(access)
    shared_semantic_metrics = _shared_semantic_metrics(access)

    canonical_identities = set(weekly_metrics.get("identities", set()))
    normalized_identities = set(tdcc_normalized_metrics.get("identities", set()))
    noncanonical_identities = sorted(normalized_identities - canonical_identities)
    tdcc_normalized_metrics["noncanonical_row_count"] = len(noncanonical_identities)
    tdcc_normalized_metrics["coverage_detail"] = (
    ...
    required_tdcc_dates = set(latest_dataset_metrics.get("required_dates", set()))
    missing_raw_dates = sorted(required_tdcc_dates - set(raw_metrics["dates"]))
    raw_metrics["coverage_detail"] = (
        "required_date_missing="
        + (";".join(missing_raw_dates) or "none")
        + f";schema_variant_count={raw_metrics.get('schema_variant_count', 0)}"
    )

    published_dates = set(snapshot_metrics["dates"])
    if published_dates:
        date_min, date_max = min(published_dates), max(published_dates)
        observed_price_dates = {
            date
            for date in daily_price_metrics["dates"]
    ...
    snapshot_metrics["coverage_detail"] = (
        snapshot_coverage_detail
        + f";transport_normalized_hash_count={snapshot_metrics['transport_normalized_hash_count']}"
        + ";non_transport_hash_mismatch_count=0"
    )
    all_candidates_metrics["coverage_detail"] = (
        snapshot_coverage_detail
        + f";transport_normalized_hash_count={all_candidates_metrics['transport_normalized_hash_count']}"
        + ";non_transport_hash_mismatch_count=0"
    )
    manifest_metrics["coverage_detail"] = (
        f"signal_revision_count={manifest_metrics['row_count']};"
    ...
    rows = [
        ...
        for family, role, path, metrics, coverage, pit in definitions
    ]
    if tuple(row["source_family"] for row in rows) != SOURCE_FAMILY_ORDER:
        raise RuntimeError("audit source family order drift")
    return rows
...
def write_audit(rows: list[dict[str, str]], output_path: Path) -> Path:
    if output_path.name != ARTIFACT_NAME:
        raise RuntimeError(f"output filename must be {ARTIFACT_NAME}")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_bytes(_csv_bytes(rows))
    return output_path


def _preflight_model_owned_output(repository_root: Path, output_path: Path) -> None:
    root = repository_root.resolve()
    resolved = output_path.resolve()
    try:
        relative = resolved.relative_to(root).as_posix()
    except ValueError as exc:
        raise RuntimeError("model-owned output path must stay within repository root") from exc
    rules = load_ownership_rules(
        root / "config" / "model_research_artifact_ownership.csv"
    )
    errors = validate_changed_paths(OWNER_ID, PRODUCER, [relative], rules)
    if errors:
        details = "\n".join(f"- {error}" for error in errors)
        raise RuntimeError(f"model-owned artifact ownership preflight failed:\n{details}")


def _nonempty_source_ref(value: str) -> str:
    source_ref = value.strip()
    if not source_ref:
        raise argparse.ArgumentTypeError(
            "--source-ref must name a non-empty commit or ref"
        )
    return source_ref
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Inventory TDCC stealth accumulation PIT replay source availability; "
            "never emit selector events or performance metrics."
        )
    )
    parser.add_argument("--repository-root", type=Path, default=ROOT)
    parser.add_argument("--source-ref", type=_nonempty_source_ref, default="HEAD")
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = args.output or root / "output" / "research" / MODEL_ID / ARTIFACT_NAME
    _preflight_model_owned_output(root, output)
    rows = build_audit_rows(root, args.source_ref)
    ...
    print(f"built_tdcc_stealth_pit_replay_availability_audit_rows={len(rows)}")
    print(f"availability_state={AVAILABILITY_STATE}")
    print(f"published_target_row_count={rows[0]['published_target_row_count']}")
