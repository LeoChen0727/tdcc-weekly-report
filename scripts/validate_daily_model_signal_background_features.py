...
def validate_catalog(panel: pd.DataFrame, catalog: pd.DataFrame) -> list[str]:
    ...
    if revenue.empty:
        errors.append("catalog must document missing monthly revenue point-in-time panel")
    elif set(revenue["feature_scope"].astype(str)) != {"missing_shared_data_family"}:
        errors.append("monthly revenue point-in-time panel must be marked missing_shared_data_family")
