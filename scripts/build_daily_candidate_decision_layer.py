...
def risk_handling_for(
    ...
    if "tdcc_distribution_warning" in flags and not attack_confirmed:
        hard_exclusion = True
    if hard_exclusion:
        return "hard_exclusion", "明確排除買進", False, True
...
def display_section_for(theme_group: str, trade_decision: str) -> str:
    if trade_decision == "no_buy":
        return "risk_no_buy"
    if trade_decision == "short_term_risk_follow":
        return f"{theme_group}_short_term_risk_follow"
    if trade_decision == "buy_if_confirmed":
        return f"{theme_group}_buy_if_confirmed"
    if trade_decision == "wait_confirmation":
        return f"{theme_group}_wait_confirmation"
    return f"{theme_group}_watch_only"
...
def write_markdown(decision: pd.DataFrame, main_date: str) -> None:
    ...
    lines.append("These rows are not front-line buys. They remain visible because high momentum plus risk can still rally and must be checked with D+5/D+10 evidence.")
    lines.append("")
