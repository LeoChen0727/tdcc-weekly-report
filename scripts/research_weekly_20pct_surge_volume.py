...
def build_stock_day_frame() -> pd.DataFrame:
    ...
    for path in sorted(PRICE_DIR.glob("*.csv")):
        ...
        future_high = df["high"].iloc[::-1].rolling(FORWARD_DAYS + 1, min_periods=1).max().iloc[::-1]
        ...
        for window in NEXT_OPEN_WINDOWS:
            # Practical entry is D+1 open.  Measure D+1..D+window
            # with vectorized reverse rolling windows instead of per-row loops.
            future_high_w = df["high"].shift(-1).iloc[::-1].rolling(window, min_periods=1).max().iloc[::-1]
            future_low_w = df["low"].shift(-1).iloc[::-1].rolling(window, min_periods=1).min().iloc[::-1]
            future_close_w = df["close"].shift(-window)
            invalid_entry = df["next_open"].isna() | (df["next_open"] <= 0)
            future_high_w = future_high_w.mask(invalid_entry)
            future_low_w = future_low_w.mask(invalid_entry)
            future_close_w = future_close_w.mask(invalid_entry)
            high_col = f"next_open_to_d{window}_high"
            low_col = f"next_open_to_d{window}_low"
            close_col = f"next_open_to_d{window}_close"
            next_open_future_cols[high_col] = future_high_w
            next_open_future_cols[low_col] = future_low_w
            next_open_future_cols[close_col] = future_close_w
