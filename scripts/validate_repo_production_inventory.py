...
VOLUME_V2_RUNTIME_MARKDOWN_WRITER_JOB = (
    "volume_v2_runtime_markdown_normalization_candidate_commit"
)
VOLUME_V2_RUNTIME_MARKDOWN_WRITER_INPUT = (
    "run_volume_v2_runtime_markdown_normalization_candidate_only"
)
VOLUME_V2_RUNTIME_MARKDOWN_ALLOWED_STAGE_PATTERNS = (
...
VOLUME_V2_RUNTIME_MARKDOWN_WRITER_IF = (
    "if: >-",
    "github.event_name == 'workflow_dispatch' &&",
    "github.run_attempt == 1 &&",
    "github.ref_type == 'branch' &&",
    "github.ref_name != 'main' &&",
    f"inputs.{VOLUME_V2_RUNTIME_MARKDOWN_WRITER_INPUT} == true &&",
    "needs.scope.result == 'success' &&",
    "needs.volume_v2_research.result == 'success'",
)
VOLUME_V2_RUNTIME_MARKDOWN_REVALIDATE_ACTIVE_SHA256 = (
    "63f693694d0ea7cea62be6eede1402b156f6c791d21922b6fd9e81a10f4ca81e"
)
...
def _child_section(lines: list[str], start: int, indent: int) -> list[str]:
    section = [lines[start]]
...
def semantic_yaml_key_count(text: str, indent: int, key: str) -> int:
...
def workflow_dispatch_input_blocks(text: str, input_name: str) -> list[str]:
    """Return only structurally nested workflow_dispatch input declarations."""
    lines = text.replace("\r\n", "\n").replace("\r", "\n").splitlines()
    blocks: list[str] = []
    for on_index, line in enumerate(lines):
        if line != "on:":
            continue
        on_lines = _child_section(lines, on_index, 0)
        for dispatch_index, dispatch_line in enumerate(on_lines):
            if dispatch_line != "  workflow_dispatch:":
                continue
            dispatch_lines = _child_section(on_lines, dispatch_index, 2)
            for inputs_index, inputs_line in enumerate(dispatch_lines):
                if inputs_line != "    inputs:":
                    continue
                inputs_lines = _child_section(dispatch_lines, inputs_index, 4)
                for input_index, input_line in enumerate(inputs_lines):
                    if input_line == f"      {input_name}:":
                        blocks.append(
                            "\n".join(_child_section(inputs_lines, input_index, 6))
                        )
    return blocks


def literal_mapping_entries(block: str, indent: int) -> tuple[dict[str, str], set[str]]:
    mapping: dict[str, str] = {}
    duplicates: set[str] = set()
    pattern = re.compile(r"^" + (" " * indent) + r"([A-Za-z0-9_-]+):\s*(.*?)\s*$")
...
def workflow_job_if_blocks(job_block: str) -> list[tuple[str, ...]]:
    lines = job_block.replace("\r\n", "\n").replace("\r", "\n").splitlines()
    blocks: list[tuple[str, ...]] = []
    for index, line in enumerate(lines):
        if not re.match(r"^    if:\s*", line):
            continue
        section = _child_section(lines, index, 4)
        blocks.append(tuple(item.strip() for item in section if item.strip()))
    return blocks
...
def workflow_steps_named(job_block: str, name: str) -> list[str]:
    return [
        step
        for step in workflow_step_blocks(job_block)
        if workflow_step_name(step) == name
    ]


def workflow_step_active_run_lines(step_block: str) -> tuple[str, ...]:
    lines = step_block.replace("\r\n", "\n").replace("\r", "\n").splitlines()
    run_index = next(
    ...
    for line in lines[run_index + 1 :]:
        if line.strip() and _line_indent(line) <= 8:
            break
        stripped = line.strip()
...
def workflow_step_active_run_sha256(active_lines: tuple[str, ...]) -> str:
...
def _require_active_lines(
    ...
    for line in required:
        if active_lines.count(line) != 1:
            errors.append(
                f"{workflow_path} writer job {job_name} {label} must contain "
                f"exactly one active command: {line}"
            )


def _validate_fail_closed_shell_guards(
    workflow_path: str,
    job_name: str,
    label: str,
    active_lines: tuple[str, ...],
    required_guards: tuple[tuple[str, str], ...],
    errors: list[str],
) -> None:
    for condition, expected_echo in required_guards:
        condition_indices = [
            index for index, line in enumerate(active_lines) if line == condition
        ]
        if len(condition_indices) != 1:
            errors.append(
                f"{workflow_path} writer job {job_name} {label} must contain "
                f"exactly one fail-closed guard: {condition}"
            )
            continue
        start = condition_indices[0]
        depth = 1
        body: list[str] = []
        for line in active_lines[start + 1 :]:
            ...
            errors.append(
                f"{workflow_path} writer job {job_name} {label} guard lacks matching fi: "
                f"{condition}"
            )
            continue
        if body != [expected_echo, "exit 1"]:
            errors.append(
                f"{workflow_path} writer job {job_name} {label} guard body must "
                f"equal only the static error echo followed by exit 1: {condition}"
            )


def _validate_no_writer_shell_masking(
    workflow_path: str,
    job_name: str,
    active_lines: tuple[str, ...],
    errors: list[str],
    ...
    protected_alternation = "|".join(map(re.escape, protected))
    assignment_re = re.compile(
        rf"^(?:export\s+)?(?:{protected_alternation})\s*="
    )
    unset_re = re.compile(rf"^unset\s+(?:{protected_alternation})(?:\s|$)")
    for line in active_lines:
        has_forbidden_exit = (
            re.search(r"(?:^|[;&|]\s*)exit(?:\s|$)", line) is not None
            and line != "exit 1"
        )
        has_return = re.search(r"(?:^|[;&|]\s*)return(?:\s|$)", line) is not None
        if (
            assignment_re.match(line)
            or unset_re.match(line)
            or re.match(r"^set\s+\+(?:e|u|o)(?:\s|$)", line)
            or line.startswith("eval ")
            or re.search(r"(?:^|\s)\|\|\s*true(?:\s|$)", line)
            or has_forbidden_exit
            or has_return
            or line in {"if false; then", "if true; then"}
        ):
            errors.append(
                f"{workflow_path} writer job {job_name} contains forbidden "
                f"identity/preflight masking command: {line}"
            )


def validate_volume_v2_runtime_markdown_pr_writer_exception(
    row: InventoryRow | None,
    text: str,
    job_blocks: dict[str, str],
    errors: list[str],
) -> None:
    """Validate the sole PR-triggered deploy-key exception without broad markers."""
    workflow_path = VOLUME_V2_RUNTIME_MARKDOWN_WRITER_WORKFLOW
    job_name = VOLUME_V2_RUNTIME_MARKDOWN_WRITER_JOB
    input_name = VOLUME_V2_RUNTIME_MARKDOWN_WRITER_INPUT

    if row is None or row.allowed_stage_patterns != VOLUME_V2_RUNTIME_MARKDOWN_ALLOWED_STAGE_PATTERNS:
        errors.append(
            f"{workflow_path} branch writer inventory allowed_stage_patterns must equal "
            "the exact two literal Volume V2 Markdown git add commands"
        )

    dispatch_count = semantic_yaml_key_count(text, 2, "workflow_dispatch")
    if dispatch_count != 1:
        errors.append(f"{workflow_path} must declare workflow_dispatch exactly once")

    input_blocks = workflow_dispatch_input_blocks(text, input_name)
    if (
        semantic_yaml_key_count(text, 6, input_name) != 1
        or len(input_blocks) != 1
    ):
        errors.append(
            f"{workflow_path} must declare workflow_dispatch input {input_name} exactly once"
        )
    else:
        input_mapping, duplicates = literal_mapping_entries(input_blocks[0], 8)
        expected_keys = {"description", "required", "default", "type"}
        if (
            duplicates
            or set(input_mapping) != expected_keys
            or input_mapping.get("required") != "false"
            or input_mapping.get("default") != "false"
            or input_mapping.get("type") != "boolean"
        ):
            errors.append(
                f"{workflow_path} workflow_dispatch input {input_name} must be one "
                "semantic boolean with required false and default false"
            )

    job_count = semantic_yaml_key_count(text, 2, job_name)
    if job_count != 1 or job_name not in job_blocks:
        errors.append(f"{workflow_path} must declare writer job {job_name} exactly once")
        return
    block = job_blocks[job_name]

    exception_candidate_jobs = {
        candidate_name
        for candidate_name, candidate_block in job_blocks.items()
        if (
            PRODUCTION_ARTIFACT_WRITE_DEPLOY_KEY in candidate_block
            or is_artifact_push_job(candidate_block)
        )
    }
    if exception_candidate_jobs != {job_name}:
        errors.append(
            f"{workflow_path} PR branch writer exception job IDs must equal only "
            f"{{{job_name}}}; observed={sorted(exception_candidate_jobs)}"
        )

    if workflow_job_if_blocks(block) != [VOLUME_V2_RUNTIME_MARKDOWN_WRITER_IF]:
        errors.append(
            f"{workflow_path} writer job {job_name} if must match the exact "
            "workflow_dispatch non-main attempt-1 boolean-input contract"
        )

    if re.search(
        r"(?:secrets\.GITHUB_TOKEN|github\.token|\bGITHUB_TOKEN\b)",
        block,
        flags=re.IGNORECASE,
    ):
        errors.append(
            f"{workflow_path} writer job {job_name} must not use a GITHUB_TOKEN fallback"
        )

    steps = workflow_step_blocks(block)
    preflights = workflow_steps_named(block, PRODUCTION_ARTIFACT_WRITE_PREFLIGHT_NAME)
    checkouts = [step for step in steps if is_checkout_step(step)]
    if len(preflights) != 1 or not is_valid_writer_secret_preflight(preflights[0]):
        errors.append(
            f"{workflow_path} writer job {job_name} must have one exact nonempty "
            "deploy-key preflight"
        )
    elif workflow_step_active_run_lines(preflights[0]) != (
        "set -euo pipefail",
        PRODUCTION_ARTIFACT_WRITE_SECRET_GUARD,
        'echo "::error::PRODUCTION_ARTIFACT_WRITE_DEPLOY_KEY is required for Volume V2 branch artifact publishing."',
        "exit 1",
        "fi",
    ):
        errors.append(
            f"{workflow_path} writer job {job_name} deploy-key preflight must match "
            "the exact fail-closed set -euo pipefail contract"
        )
    if len(checkouts) != 1:
        errors.append(
            f"{workflow_path} writer job {job_name} must have exactly one deploy-key checkout"
        )
    else:
        checkout_with = workflow_step_mapping(checkouts[0], "with")
        expected_checkout_with = {
            "fetch-depth": "0",
            "ref": "${{ inputs.expected_head_sha }}",
            "ssh-key": PRODUCTION_ARTIFACT_WRITE_SECRET_EXPRESSION,
            "persist-credentials": "true",
        }
        if checkout_with != expected_checkout_with:
            errors.append(
                f"{workflow_path} writer job {job_name} checkout must use only the "
                "exact expected head and deploy-key SSH credentials"
            )
    if preflights and checkouts and steps.index(preflights[0]) >= steps.index(checkouts[0]):
        errors.append(
            f"{workflow_path} writer job {job_name} must check the deploy key before checkout"
        )

    revalidate_name = "Revalidate exact Volume V2 code commit scope"
    revalidate_steps = workflow_steps_named(block, revalidate_name)
    if len(revalidate_steps) != 1:
        errors.append(
            f"{workflow_path} writer job {job_name} must have one {revalidate_name} step"
        )
    else:
        revalidate = revalidate_steps[0]
        expected_env = {
            "EXPECTED_BASE_SHA": "${{ inputs.expected_base_sha }}",
            "EXPECTED_HEAD_SHA": "${{ inputs.expected_head_sha }}",
            "CONFIRMATION": "${{ inputs.confirmation }}",
        }
        if workflow_step_mapping(revalidate, "env") != expected_env:
            errors.append(
                f"{workflow_path} writer job {job_name} must bind exact base/head inputs"
            )
        revalidate_lines = workflow_step_active_run_lines(revalidate)
        if (
            workflow_step_active_run_sha256(revalidate_lines)
            != VOLUME_V2_RUNTIME_MARKDOWN_REVALIDATE_ACTIVE_SHA256
        ):
            errors.append(
                f"{workflow_path} writer job {job_name} revalidation active body "
                "must match the exact canonical SHA-256 contract"
            )
        _validate_no_writer_shell_masking(
            workflow_path,
            job_name,
            revalidate_lines,
            errors,
        ...
        revalidate_guards = (
            (
                ...
                'echo "Volume V2 normalization commit requires a non-main branch dispatch" >&2',
            ),
            (
                'if [ "$GITHUB_RUN_ATTEMPT" != "1" ]; then',
                'echo "Volume V2 normalization commit rejects rerun attempts" >&2',
            ),
            (
                'if [ "$CONFIRMATION" != "normalize_volume_v2_runtime_markdown_candidate" ]; then',
                'echo "Volume V2 normalization confirmation mismatch" >&2',
            ),
            (
                'if [ "$GITHUB_SHA" != "$EXPECTED_HEAD_SHA" ] || [ "$checkout_sha" != "$EXPECTED_HEAD_SHA" ] || [ "$remote_branch_sha" != "$EXPECTED_HEAD_SHA" ]; then',
                'echo "Volume V2 normalization code head moved before commit" >&2',
            ),
            (
                'if [ "$parent_sha" != "$EXPECTED_BASE_SHA" ]; then',
                'echo "Volume V2 normalization code-parent identity mismatch before commit" >&2',
            ),
        )
        revalidate_guard_conditions = tuple(
            condition for condition, _ in revalidate_guards
        )
        _require_active_lines(
            workflow_path,
            job_name,
    ...
    commit_name = "Commit and push exact Volume V2 Markdown normalization candidate"
    commit_steps = workflow_steps_named(block, commit_name)
    if len(commit_steps) != 1:
        errors.append(
            f"{workflow_path} writer job {job_name} must have one exact2 commit/push step"
        )
        return
    commit_step = commit_steps[0]
    if workflow_step_mapping(commit_step, "env") != {
        "EXPECTED_HEAD_SHA": "${{ inputs.expected_head_sha }}"
    }:
        errors.append(
            f"{workflow_path} writer job {job_name} commit step must bind expected head"
        )
    active_lines = workflow_step_active_run_lines(commit_step)
    if (
        workflow_step_active_run_sha256(active_lines)
        != VOLUME_V2_RUNTIME_MARKDOWN_COMMIT_ACTIVE_SHA256
    ):
        errors.append(
            f"{workflow_path} writer job {job_name} commit/push active body must "
            "match the exact canonical SHA-256 contract"
        )
    _validate_no_writer_shell_masking(
        workflow_path,
        job_name,
    ...
    git_add_lines = tuple(
        line for line in active_lines if re.match(r"^git\b.*\badd(?:\s|$)", line)
    )
    if git_add_lines != VOLUME_V2_RUNTIME_MARKDOWN_ALLOWED_STAGE_PATTERNS:
        errors.append(
            f"{workflow_path} writer job {job_name} must stage only the exact two "
            "literal Volume V2 Markdown paths"
        )
    git_commit_lines = tuple(
        line for line in active_lines if re.match(r"^git\b.*\bcommit(?:\s|$)", line)
    )
    expected_commit = (
        'git --no-replace-objects commit -m "Normalize Volume V2 runtime audit Markdown"',
    )
    if git_commit_lines != expected_commit:
        errors.append(
            f"{workflow_path} writer job {job_name} must create exactly one artifact commit"
        )
    git_push_lines = tuple(
        line for line in active_lines if re.match(r"^git\b.*\bpush(?:\s|$)", line)
    )
    expected_push = (
        'git --no-replace-objects push origin "HEAD:refs/heads/$GITHUB_REF_NAME"',
    )
    if git_push_lines != expected_push:
        errors.append(
            f"{workflow_path} writer job {job_name} must push only HEAD to the exact dispatched branch"
        )
    commit_guards = (
        (
            'if [ "$artifact_parent_sha" != "$EXPECTED_HEAD_SHA" ] || [ "$(git --no-replace-objects rev-list --count "$EXPECTED_HEAD_SHA..$artifact_commit_sha")" != "1" ]; then',
            'echo "Volume V2 artifact commit must be the code head\'s unique direct child" >&2',
        ),
        (
            'if [ "$remote_branch_sha" != "$EXPECTED_HEAD_SHA" ]; then',
            'echo "Remote branch moved before Volume V2 artifact push" >&2',
        ),
        (
            'if [ "$published_sha" != "$artifact_commit_sha" ]; then',
            'echo "Remote branch does not equal the Volume V2 artifact commit" >&2',
        ),
    )
    commit_guard_conditions = tuple(condition for condition, _ in commit_guards)
    _require_active_lines(
        workflow_path,
        job_name,
        "artifact direct-child publication",
        active_lines,
        (
            "set -euo pipefail",
            'artifact_commit_sha="$(git --no-replace-objects rev-parse HEAD)"',
            'artifact_parent_sha="$(git --no-replace-objects rev-parse HEAD^)"',
            *commit_guard_conditions,
            'remote_branch_sha="$(git --no-replace-objects ls-remote origin "refs/heads/$GITHUB_REF_NAME" | awk \'{print $1}\')"',
            'published_sha="$(git --no-replace-objects ls-remote origin "refs/heads/$GITHUB_REF_NAME" | awk \'{print $1}\')"',
        ),
        errors,
    )
    _validate_fail_closed_shell_guards(
...
def validate_production_artifact_writer_auth(
    ...
    for workflow_path in sorted(workflow_paths):
        ...
        exception_writer_jobs: dict[str, str] = {}
        if workflow_path == VOLUME_V2_RUNTIME_MARKDOWN_WRITER_WORKFLOW:
            validate_volume_v2_runtime_markdown_pr_writer_exception(
                row,
                text,
                job_blocks,
                errors,
            )
            exception_block = job_blocks.get(VOLUME_V2_RUNTIME_MARKDOWN_WRITER_JOB)
        ...
        if (
            workflow_has_pull_request_trigger(text)
            and PRODUCTION_ARTIFACT_WRITE_DEPLOY_KEY in text
            and workflow_path != VOLUME_V2_RUNTIME_MARKDOWN_WRITER_WORKFLOW
        ):
            errors.append(
