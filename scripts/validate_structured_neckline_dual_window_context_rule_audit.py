...
def validate_rule_ids(frame: pd.DataFrame, label: str) -> None:
    got = sorted(set(frame["candidate_rule_id"].astype(str)))
    if got != sorted(RULE_IDS):
        fail(f"{label} candidate_rule_id must be {RULE_IDS}; got {got}")


def validate_detail(detail: pd.DataFrame) -> None:
    require_columns(detail, DETAIL_COLUMNS, "detail")
    validate_constants(detail, "detail")
    validate_rule_ids(detail, "detail")

    source_events = detail["source_event_key"].nunique()
    if source_events < 300:
        fail(f"detail source events too small: {source_events}")
    expected_rows = source_events * len(RULE_IDS)
    if len(detail) != expected_rows:
        fail(f"detail rows must equal source_events * rule_count; got {len(detail)} vs {expected_rows}")

    accepts = set(detail["candidate_accept"].astype(str))
    if not accepts <= {"true", "false"}:
        fail(f"candidate_accept has unexpected values: {sorted(accepts - {'true', 'false'})}")
    if not detail.groupby("candidate_rule_id")["candidate_accept"].apply(lambda s: (s == "true").any()).all():
        fail("every candidate rule must accept at least one row")
...
def validate_summary(summary: pd.DataFrame) -> None:
    ...
    validate_rule_ids(summary, "summary")
    ...
    if "accepted_by_long_window_risk" not in set(summary["summary_scope_id"].astype(str)):
        fail("summary must include accepted_by_long_window_risk")
...
def validate_manual_alignment(alignment: pd.DataFrame) -> None:
    ...
    validate_rule_ids(alignment, "manual_alignment")
...
def main() -> int:
    ...
    print(
        "structured neckline dual window context rule audit validation passed "
