...
AUDIT_ID = "financial_statement_historical_pit_source_audit_v4"
...
def markdown(rows: list[dict[str, str]]) -> str:
    ...
    lines = [
        ...
        "TWSE Data E-Shop identifies an official MOPS active-push candidate in package 5 item S21 `IFRS財報檔案`, while package 2 identifies L01 `每日傳送清單檔`. The public product pages do not publish a field schema, 2013Q1 historical coverage, server received or public-availability timestamps, revision sequence, supersedes key, immutable payload identifier, or checksum. The official application page requires direct contact with the TWSE Digital and Cybersecurity Department. These pages prove that a formal acquisition channel exists; they do not prove that S21 or L01 can reconstruct historical PIT.",
    ...
    lines.extend(
        [
            ...
            "",
            "The next admissible acquisition step is a user-authorized request to TWSE Data E-Shop for the written S21 and L01 specifications plus a sample covering company id, report period, statement scope, taxonomy version, submission received time, public availability time, revision sequence, supersedes key, immutable payload or official checksum, and 2013Q1 onward retention. No external contact, registration, purchase, or order has been made by this audit.",
            "The next admissible implementation is a source adapter that can bind each company-period-scope revision and every exposed fact state to an immutable payload SHA-256 plus its official availability timestamp or date. A PDF upload time cannot be reused as XBRL revision time; a material-information time applies only to the metrics actually present in that announcement. Statutory deadlines, audit/review dates, ZIP timestamps, product-page capture times, and first-observed local capture times are not substitutes.",
            "",
        ]
    )
    return "\n".join(lines)
